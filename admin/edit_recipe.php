...
<div class="container animate-on-scroll" style="padding-top:6rem;">
  ...
  <form method="post" enctype="multipart/form-data" class="animate-on-scroll" style="max-width:600px; margin-top:2rem;">
    ...
    <label class="animate-on-scroll">Chef:<br>
      <select name="chef_id" required class="form-input">
        <?php while($c = $chefs->fetch_assoc()): ?>
          <option value="<?= $c['id'] ?>" <?= $c['id'] == $recipe['chef_id'] ? 'selected' : '' ?>>
    ...
    <label class="animate-on-scroll">Current Image:<br>
      <?php if ($recipe['image_path']): ?>
        <img src="/COM6011/images/recipes/<?= htmlspecialchars($recipe['image_path']) ?>" alt="" class="animate-on-scroll" style="max-width:100px; display:block; margin-bottom:0.5rem;">
      <?php else: ?>
    ...
    <a href="recipes.php" class="admin-btn animate-on-scroll">Go Back</a>
  </form>
</div>
